from GNN_models.finalmlp import FinalPredMLP
from GNN_models.selfatten import SelfAttentionLayer
from GNN_models.subgraph import SubGraph
import os
import pdb
...
class HGNN(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, goal_plus_lastSpeed_shape=4,num_subgraph_layers=2, num_global_graph_layer=1, subgraph_width=32, global_graph_width=32, final_mlp_hidden_width=64):
    ...
    def forward(self, data):
        ...
        out_new=torch.cat((out[:, [0]].squeeze(1),data.goal.view(-1,2),data.last_speed.view(-1,2)),dim=1)
        pred = self.traj_pred_mlp(out_new)
        return pred
    


class HGNN_Disrim(nn.Module):
    """
    hierarchical GNN with trajectory prediction MLP
    """
    ...
    def forward(self, state, action):
        ...
        x = sub_graph_out.x.view(-1, time_step_len, self.polyline_vec_shape)
        out = self.self_atten_layer(x, valid_lens)
        out_new=torch.cat((out[:, [0]].squeeze(1), state.goal.view(-1,2), state.last_speed.view(-1,2), action.view(-1,2)), dim=1)
        pred = self.traj_pred_mlp(out_new)
        return pred